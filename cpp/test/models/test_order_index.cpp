...
TEST_F(OrderIndexTest, ConstructorAndGetters) {
    ...
    OrderIndex index1(Side::Buy, order1->getPriceTicks(), it1);
    OrderIndex index2(Side::Sell, order2->getPriceTicks(), it2);

    EXPECT_EQ(index1.getSide(), Side::Buy);
    EXPECT_EQ(index1.getPriceTicks(), 10ull);
    EXPECT_EQ(index1.getOrderIter(), it1);
    EXPECT_EQ(index2.getSide(), Side::Sell);
